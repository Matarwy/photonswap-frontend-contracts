...
export const useTotalValue = (): BigNumber => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      if (pool.stakingTokenName === QuoteToken.CAKE) {
        const totalCakeStaked = new BigNumber(pool.totalStaked).div(new BigNumber(10).pow(18))
        poolValue = cakePrice.times(totalCakeStaked)
      }
  ...
  }, [bnbPrice, ethPrice, farms, pools, cakePrice])
